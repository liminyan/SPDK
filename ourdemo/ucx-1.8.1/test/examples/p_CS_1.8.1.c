...
static int send_recv_stream(ucp_worker_h ucp_worker, ucp_ep_h ep, int is_server,
                            int current_iter, void* buffer, int comm_size)
{
    ...
    return request_finalize(ucp_worker, request, is_server, buffer,
                            current_iter);
}
									   ...
									   void* buffer, int comm_size)
{
    ...
    case CLIENT_SERVER_SEND_RECV_STREAM:
        ...
        ret = send_recv_stream(worker, ep, is_server, current_iter, buffer, comm_size);
        break;
                                 ...
                                 send_recv_type_t send_recv_type, int is_server)
{
  ...
  //zzz
    ...
    for (exp = 0 ; exp < 13 ; exp++){

        data_size = pow(2,exp);
        for (i = 0 ; i < data_size ; i++) buffer[i] = 'a';
        
        gettimeofday(&start, 0);
        for (i = 0; i < num_iterations; i++) {
            ret = client_server_communication(ucp_worker, ep, send_recv_type,
                                              is_server, i, buffer, data_size);
            if (ret != 0) {
        ...
        gettimeofday(&end, 0);
        time = TIME(start , end);
        printf("%d %.4lf MB/s\n", data_size, data_size * num_iterations / 1000 / 1000 / time);
    }
